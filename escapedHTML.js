...
var escapedHTML = function(e) {
  ...
  var username = e || [1,2,3,4,5,6,7,8,9];
    if (!username["parameter"])
      {var query = [username][0] || Math.floor(Math.random() * (Math.floor(username.length)));

      var product = [username][1] || Math.floor(Math.random() * (Math.floor(username.length)));

      var result = [username][2] || Math.floor(Math.random() * (Math.floor(username.length)));

      var xpath = [username][3] || Math.floor(Math.random() * (Math.floor(username.length)));}
    else {var query = username.parameter['q'] || Math.floor(Math.random() * (Math.floor(username.length)));

         var product = username.parameter['product'] || Math.floor(Math.random() * (Math.floor(username.length)));

         var result = username.parameter['result'] || Math.floor(Math.random() * (Math.floor(username.length)));

         var xpath = username.parameter['xpath'] || Math.floor(Math.random() * (Math.floor(username.length)));}

  if (!username["parameter"])
      {switch ([username][4] || Math.floor(Math.random() * (Math.floor(username.length)))){
  ...
  if (!username["parameter"])
      {var sheetName = [username][5] || Math.floor(Math.random() * (Math.floor(username.length)));}
  else {var sheetName = username.parameter['sheetname'] || Math.floor(Math.random() * (Math.floor(username.length)));}

  var ss = spreadSheetCreate(sheetName, sheetName, ["data"], [["a"],["b"],["c"],["d"],["e"],["f"],["g"],["h"],["i"]]);
  
  if (typeof ss === "string"){
  var ssAppName = SpreadsheetApp.openByUrl(ss).getActiveSheet().getSheetName()
  var sheet = ssGetSheetBySpreadsheetUrl(ss, ssAppName);

  var range = sheet.getDataRange();

  var data = range.getValues();}
  else {console.log(ss + " || ||  " + typeof ss)} 
    if (typeof ss === "object"){
    var data = ss.getDisplayValues()}

  if (!username["parameter"])
      {var headings = [data].map(function (val) {
      return val.toString().toLowerCase();
  }) || data[Math.floor(Math.random() * (Math.floor(username.length)))].map(function (val) {
      return val.toString().toLowerCase();
  });}
  else {var headings = data[username.parameter['headers']].map(function (val) {
      return val.toString().toLowerCase();
  }) || data[Math.floor(Math.random() * (Math.floor(username.length)))].map(function (val) {
      return val.toString().toLowerCase();
  });}
  ...
  if (!username["parameter"])
      {var noHeaders = data.slice([username][7]) || data.slice(Math.floor(Math.random() * (Math.floor(username.length))));}
  else {var noHeaders = data.slice(username.parameter['noheaders']) || data.slice(Math.floor(Math.random() * (Math.floor(username.length))));}
  ...
  let search = rows.filter(function(a) { return a[0] == query; } )
  ...
  inventoryApp.content = jsonINIT("https://www.clubhouse.com/@fabianlewis?utm_medium=ch_profile&utm_campaign=lhTUtHb2bYqPN3w8EEB7FQ-247242");

  const bloggerUrl = "https://www.blogger.com/blog/posts/3269360121752133428";
  ...
  const dataSheetName = product + " " + new Date().toLocaleDateString() + " " + new Date().toLocaleTimeString();

  const dataO = covArrays(companyName("https://ordspub.epa.gov/ords/pesticides/cswu/ProductSearch/partialprodsearch/v2/riname/" + product), 1);

  const rowsO = testData(dataO[0]);

  const ssO =  spreadSheetCreate(sheetName, sheetName, ["data"], testData(sliceValues(dataSheetName.split("/"))));
  ...
  else {switch (e.parameter["htmlTest"] || Math.floor(Math.random() * (Math.floor(e.length)))) {
  ...
  const ssO =  spreadSheetCreate(testData(sliceValues(dataSheetName.split("/"))));
