function styleHtml() {
  ...
  content.utf_8 = HtmlService.createHtmlOutput(`
    <meta charset="UTF-8">`).getContent();
  content.viewport = HtmlService.createHtmlOutput(`
    <meta name="viewport" content="width=device-width, initial-scale=1.0">`).getContent();
  content.fontAwesome = HtmlService.createHtmlOutput(`
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.1.1/css/all.min.css" integrity="sha512-KfkfwYDsLkIlwQp6LFnl8zNdLGxu9YAA1QvwINks4PhcElQSvqcyVLLD9aMhXd13uQjoXtEKNosOWaZqXgel0g==" crossorigin="anonymous" referrerpolicy="no-referrer" />`).getContent();
  content.google_charts_loader = HtmlService.createHtmlOutput(`
    <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>`).getContent();
  content.google_signin_client = HtmlService.createHtmlOutput(`
    <meta content="739921544160-nvqbr8cmqcs35n700q94mn5qnjh7vdr5.apps.googleusercontent.com" name="google-signin-client_id"></meta>`).getContent();
  content.googleApisOnApiLoad = HtmlService.createHtmlOutput(`
    <script src="https://apis.google.com/js/api.js?onload=onApiLoad"></script>`).getContent();
  content.googleApis_defer = HtmlService.createHtmlOutput(`
    <script async="" defer="" src="https://apis.google.com/js/platform.js"></script>`).getContent();
  content.googleApis_preConnect = HtmlService.createHtmlOutput(`
    <link rel="preconnect" href="https://fonts.googleapis.com">`).getContent();
  content.gstatic_preConnect = HtmlService.createHtmlOutput(`
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>`).getContent();
  content.googleApisCss = HtmlService.createHtmlOutput(`
    <link href="https://fonts.googleapis.com/css2?family=Libre+Barcode+128&family=Montserrat:ital@1&family=Oswald&family=Roboto&display=swap" rel="stylesheet">`).getContent();
  ...
  content.tabulatorCss = HtmlService.createHtmlOutput(`
    <link href="https://unpkg.com/tabulator-tables@5.2.3/dist/css/tabulator_materialize.min.css" rel="stylesheet">`).getContent();
  content.googleApisIcon = HtmlService.createHtmlOutput(`
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet"></link>`).getContent();
  ...
  content.luxonJs = HtmlService.createHtmlOutput(`
    <script src="https://cdnjs.cloudflare.com/ajax/libs/luxon/3.0.1/luxon.min.js" integrity="sha512-6ZJuab/UnRq1muTChgrVxJhSgygmL2GMLVmSJN7pcBEqJ1dWPbqN9CiZ6U3HrcApTIJsLnMgXYBYgtVkJ8fWiw==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>`).getContent();
  content.tabulatorJs = HtmlService.createHtmlOutput(`
    <script type="text/javascript" src="https://unpkg.com/tabulator-tables@5.2.3/dist/js/tabulator.min.js"></script>`).getContent();
  content.jsQuery = HtmlService.createHtmlOutput(`
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>`).getContent();
  content.website = HtmlService.createHtmlOutput(`
    * {box-sizing: border-box;margin: 0;
      padding: 0;font-family: "Roboto", sans-serif;}`).getContent();
  content.html = HtmlService.createHtmlOutput(`
    html,
    body {height: 100%;}`).getContent();
  content.main = HtmlService.createHtmlOutput(`
    main {height: 92%;
          margin-top: 10px;}`).getContent();
  content.body = HtmlService.createHtmlOutput(`
    body {background-color: #ffc107;}`).getContent();
  content.header = HtmlService.createHtmlOutput(`
    header {background-color: rgba(255,255,255,.1);
            color: #a7e1ee;
            font-size: smaller;}`).getContent();
  content.header_h1 = HtmlService.createHtmlOutput(`
    header h1 {font-family: "Montserrat", cursive;
              margin-left: 15px;}`).getContent();
  content.grid = HtmlService.createHtmlOutput(`
    .grid {display: grid;}`).getContent();
  content.flex_row = HtmlService.createHtmlOutput(`
    .flex-row {display: flex;
              flex-direction: row;}`).getContent();
  content.flex_column = HtmlService.createHtmlOutput(`
    .flex-column {display: flex;
                  flex-direction: column;}`).getContent();
...
var frameHtml 
= function(file, argsArray) {const content 
= {};content.iframePlayer 
  = function fPla() {const myObj 
  ...
  if (!window && !window.myApp) {window.myApp 
  = {}};window.myApp.serverSide 
  = myObj.appJS; // Or, if you have a namespace
  // : myApp.serverSide 
  // = serverSide;
  const tag 
  = document.createElement("script");tag.src 
  = "https://www.youtube.com/iframe_api";const firstScriptTag 
  = document.getElementsByTagName("script")[0];firstScriptTag.parentNode.insertBefore(tag, firstScriptTag);let iframePlayer;function onYouTubeIframeAPIReady() {iframePlayer 
  = new YT.Player("iframePlayer", {height: "505",  width: "585", 
                 //  videoId: 'OTeQee-gxa4', 
                 playerVars: {"autoplay": 1, "loop": 1, "controls": 0, "showinfo": 0, "autohide": 1, "playsinline": 1, "mute": 1, "modestbranding": 1, "vq": "hd1080", "listType": 'playlist', "list": 'UU6DOFpA9UCTgNwJiVX1IOpQ',},  
                 events: {"onReady": onPlayerReady, "onStateChange": onPlayerStateChange,}});function onPlayerReady(event) {event.target.playVideo();};const done 
  = false;function onPlayerStateChange(event) {if (event.data == YT.PlayerState.PLAYING) {setTimeout(playVideo);// done 
  // = true;
  }};function stopVideo() {iframePlayer.stopVideo();};function playVideo() {iframePlayer.playVideo();};function nextVideo() {iframePlayer.nextVideo();}};
  return  myObj};
  return content}
