function  surveyPlayer(searchString, joinString) {console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + "\nsearchString is !" + !searchString + " = " + searchString + "\njoinString is !" + !joinString + " = " + joinString)
  //var url = seoSheet(encodeURIComponent(searchString)).url;Youtube Widget
  var eFolder 
  = DriveApp.getFoldersByName("SDTLC").next();var folderFile 
  = eFolder.getFilesByName("Code Editor");var dataTree 
  = driveDocBrowser();while (folderFile.hasNext()) {var myFile 
  = folderFile.next();var currentFileUrl 
  =  myFile.getUrl()}; //  dataTree.push(currentFileUrl)};
  var testGen 
  = testlt();var listGen 
  ...
  = substanceVegas(0,[listGen].toString().length,[listGen].toString().split(""),50,1,20).substHost;noList.push(noVid)};
  var rndFiled 
  = Math.floor(Math.random() * (Math.floor(dataTree.length)));var filed 
  = dataTree[rndFiled];if (!searchString && !joinString) {var numVarRnd 
  = randNum(arguments.callee.name);var arrDRnd 
  = appSort(numVarRnd);var searchString 
  = randomSubstance(0,6,arrDRnd).myNewArr;var joinString 
  = [searchString].join("")};if (!joinString) {var joinString 
  = [searchString].join("")};var uti 
  = dtlsSomeFunction(searchString).playList;const html 
  ...
  = stream;eventL.innerText 
  = "Research"}}}).catch((er) => {eventL.innerText 
  = er})}}})})</script></body></html>`);if (uti) {var idArray 
  = uti.map((piece) => {try {
  //   console.log(typeof piece)
  if (typeof piece  !== "undefined") {
  // console.log("it's " + typeof piece)
  return piece }}catch(error){return "Error in surveyPlayer " + error}})}else {try{uti.map((piece) => {return piece})}catch(error){var idArray 
  = [error].toString().split(" ")}};html.myPlayList 
  = [idArray].sort((a, b) => a - b);html.link 
  = filed;html.searchTtile 
  = joinString;html.home 
  = getScriptUrl();html.reValue 
  = (() => {return noList[Math.floor(Math.random() * (Math.floor(noList.length)))]})();return html.evaluate().getContent();}
