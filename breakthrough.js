...
function rndChoice(game) {
  ...
  var form = FormApp.create("Coin Trial")
    .setDescription(trial.length + " Tosses\n" + cArr)
  ...
  trial.map((seo) => {
    ...
    if (seo["heads"]) {
      ...
      if (headStart === 1) {
        ...
        while (
          upscale === 0 ||
          (Number(seo["heads"]) >= 10 && upscale < 10) ||
          (Number(seo["heads"]) >= 20 && upscale < 20) ||
          (Number(seo["heads"]) >= 30 && upscale < 30) ||
          (Number(seo["heads"]) >= 40 && upscale < 40) ||
          (Number(seo["heads"]) >= 50 && upscale < 50) ||
          (Number(seo["heads"]) >= 60 && upscale < 60) ||
          (Number(seo["heads"]) >= 70 && upscale < 70) ||
          (Number(seo["heads"]) >= 80 && upscale < 80) ||
          (Number(seo["heads"]) >= 90 && upscale < 90)
        ) {
          upscale = Math.floor(
            Math.random() * Math.floor(Number(seo["heads"]) + 1),
          );
...
function rndDice() {
  ...
  var fives = 0;
  var sixes = 0;
  var multq = Math.floor(Math.random() * 6);
  ...
  var form = FormApp.create("Dice Roll")
    .setDescription(judge + " Rolls")
    .setDescription(rule());
  var formUrl = form.getPublishedUrl();
  for (var key in trial) {
    form
      .addSectionHeaderItem()
      .setTitle(randomSubstance(0, 1, [trial[key]]).myNewArr)
      .setHelpText(key);
  }
  return formUrl;
}
