  ...
  var fileList = [] 
    ...
    while (lowList.length === 0) {
      ...
      for (i=0,l=fileList.length;i<l;i++) {
        var lowListEx = fileList[i].toLowerCase().indexOf([filterFiles].join("").toLowerCase()) > -1;
        if (lowListEx)  {
          lowList.push(fileList[i])
        }
      }
      for (i=0,l=fileList.length;i<l;i++){
        var lowListIn = fileList[i].toLowerCase().includes([filterFiles].join("").toLowerCase());
        if (lowListIn)  {
...
var driveDocBrowser = function() {
  ...
  var file 
    = files.next(); 
  docsList.push(file.getUrl())};return docsList}
...
var fileBrowser = function(folder) {	
  ...
  var fileFiller = function(folder) {
    ...
    console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\nfilesObj.length: " + filesObj.length);
    var glFiles = [];
    for (var key in filesObj) {
      glFiles.push(filesObj[key])}
    var glMov = glFiles[Math.floor(Math.random() * (Math.floor(glFiles.length)))]
    var obMov = filesObj[Math.floor(Math.random() * (Math.floor(filesObj.length)))]
    console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\nglFiles.length: " + glFiles.length)
    if (file) {
      if (glFiles.join("").toLowerCase().indexOf([file].join("").toLowerCase()) === -1 && glFiles.length > 0 || filesObj.join("").toLowerCase().indexOf([file].join("").toLowerCase()) === -1 && filesObj.length >= 1) {
        console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\nMoving file = " + file + " to file = " + glMov || obMov)
        file = glMov || obMov}
      else {
        return}}
    else {
  ...
  if (typeof folder === "undefined" || folder === null) {
    ...
    console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\nglFolders/folderManager().length: " + glFolders.length)
    // if (glFolders.indexOf(folder) === -1) {folder} || glFolders.indexOf(folder) === -1}
    var folder = glFolders[Math.floor(Math.random() * (Math.floor(glFolders.length)))]
    file = fileFiller(folder)}
  else {
    foldersObj = folderManager(folder)
    var folder = foldersObj[Math.floor(Math.random() * (Math.floor(foldersObj.length)))] 
    console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\nfoldersObj/folderManager(" + folder + ").length: " + foldersObj.length);
    file = fileFiller(folder)}}
      //  || glFolders.indexOf(folder) !== -1
     if (typeof folder !== "undefined" || folder === null || [folder].join("").length > 0) {
      console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\nfolder: " + folder)
     ...
     var dataTree = [] 
     if (typeof file !== "undefined" || file !== null || [file].join("").length > 0 || glFiles.indexOf(file) !== -1) {
      console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\nfile: " + file)
      var folderFile = eFolder.getFilesByName(file)
       while (folderFile.hasNext()) { 
      var myFile = folderFile.next()
    ...
    console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\n" + dataTree)
     var rndFiledMain = Math.floor(Math.random() * (Math.floor(dataTree.length))) 
     var filedMain = dataTree[rndFiledMain]
  return console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\n" + filedMain)
}
...
var fileFold = function(folderX, fileX,  time) {
  ...
  var fileFree = []
  if (typeof folderX !== "undefined" || folderX !== null || [folderX].join("").length > 0) {
...
var fileManager = function(fileX, folder, time, content, mimeType) {
  ...
  if (typeof fileX !== "undefined" && typeof folder !== "undefined") {
    console.log("Declaring folderId calling folderIdGlobal(" + folder + ", time)")
    ...
    console.log("Declaring file calling DriveApp.getRootFolder().getFilesByName(" + DriveApp.getFileById(fileX).getName() + ").next() to search the root folder for the file")
    var file = DriveApp.getRootFolder().getFilesByName(DriveApp.getFileById(fileX).getName());
    ...
    if (myFile) {
      // console.log(myFile)
      console.log("calling DriveApp with id of file found in root drive - " + folderId)
      DriveApp.getFileById(myFile.getId()).moveTo(DriveApp.getFolderById(folderId))
      return}
    else {
...
var fileMatchManager = function(folderX, fileX,  time) {
  ...
  var fileTree = []
  if (typeof folderX !== "undefined" || folderX === null || [folderX].join("").length > 0) {
    console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\nfolderX: " + "\nA: type of folder - " + typeof folderX + "\nB: folderX - " + folderX + "\nC: folder length - " + [folderX].join("").length)
...
var furtFolder = function(folder) {
  ...
  console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\nfoldersObj.length: " + foldersObj.length + "\nfoldersObj");
  var glFolders = [];
  for (var key in foldersObj) {
    glFolders.push(foldersObj[key])}
  console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + ":\nglFolders.length: " + glFolders.length + "\nglFolders")
  if (typeof folder === "undefined" || folder === null || [folder].join("").length === 0 || glFolders.indexOf(folder) === -1) {
    var folder = glFolders[Math.floor(Math.random() * (Math.floor(glFolders.length)))]}
  return folder}
...
var rndUrls = function() {
  ...
  while (treeRoot.hasNext()) {
    ...
    console.log(trueName.getName() + " || / || " + trueName.getName()[Object.keys(trueName.getName())[Math.floor(Math.random() * (Math.floor(Object.keys(trueName.getName()).length)))]])
  var truArray = []
