function proMediaSnip(e) {
  ...
  html.artifacts = HtmlService.createTemplate(
    `<nav><a href="" target="_blank">Lab Results</a><a href="chemo" target="_blank">Style</a><a href="homeGame" target="_blank">Play</a><a href="media" target="_blank">Media</a></nav><div class="row"><div class ="col s12"><div class="container"><div class="col s12"><?!= geneFrame(seoPastTime(media).playList[0]) ?></div></div></div></div>`,
  ).getRawContent();
  html.artifacts.media = media;
  html.artifacts.odd = chancing;
  html.artifacts.play = gaming; // html.artifacts.htmlStyle = htmlCss;
...
function prose() {
      ...
      if (Array.isArray(infoLink)) {
        var page = infoLink.map(function (tv) {
          ...
          if (Array.isArray(listItem)) {
            ...
            console.error(listItem + " listItem is not an array");
          }
        });
      } else {
        console.error(infoLink + " infoLink.keyWords is not an array.");
      }
      var story =
  ...
  var result = myApp.appJS();
  if (result) {
    Logger.log("Result: " + result);
  } else {
    Logger.log("Error: " + result);
  }
  var htmlOutput = `<!DOCTYPE html><html id="proMedia"><head><base target="_top"><meta charset="utf-8"><meta name="proMedia" content="Boilerplate Professional Media Services"><meta name=viewport content="width=device-width, initial-scale=1"><link href="https://fonts.googleapis.com/css?family=Acme" rel="stylesheet"><style>
