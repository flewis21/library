...
function jsonToSpreadsheet(data, time) {
  data
    ? data
    : (data = JSON.parse(
        convertToObjects([[testlt()]], ["name"], autoGlobe.functionRegistry.time),
      )[0]);
  console.log(
...
function jsonXCalc(searchString, time) {
  ...
  var isProduct = DriveApp.getFileById(
...
function jsonXSpreadsheet(rndTitle, time) {
  ...
  const uniqueCo = jsonXCalc(rndTitle, autoGlobe.functionRegistry.time);
...
var seoBites = function (searchString, idArray, time) {
  ...
  searchString
    ? searchString
    : (searchString = autoGlobe.searchString().myNewArr);
...
var seoIndex = function (searchWord) {
  if (typeof searchWord === "undefined") {
    searchWord = autoGlobe.searchString().myNewArr;
  }
  console.log(
...
function seoPictTime(searchString, time) {
  let executed = 0;
  autoGlobe.functionRegistry.imgTree();
  var imgSheetVals = autoGlobe.functionRegistry.getImageList();
  ...
  searchString
    ...
    : (searchString = objectOfS(
        ["parameter"],
        [[["func", autoGlobe.functionRegistry.fileList[Math.floor(Math.random() * autoGlobe.functionRegistry.fileList.length)]]]],
        autoGlobe.functionRegistry.time,
      ));
  console.log("searchString = " + searchString, executed++);
  if (typeof searchString === "object") {
  ...
  if (fndOrd) {
    ...
    if (rndRes.length > 0) {
      // autoGlobe.functionRegistry.imgTree();
      // imgSheetVals = autoGlobe.functionRegistry.getImageList();
      // console.log("imgSheetVals = " + imgSheetVals, executed++);
      imgData = [];
    ...
    imgKeys.forEach((img) => {
      ...
      matchKeys.forEach((match) => {
        ...
        let truMatch = autoGlobe.trueVfalse(imgMatch);
...
function seoPictures(searchString, time) {
  ...
  if (!data) {
    autoGlobe.functionRegistry.getDomainList();
    let domainEngine = [];
     let mainList= autoGlobe.functionRegistry.arrDomainVals;
     console.log("mainList = " + JSON.stringify(mainList.slice(mainList.length - 1)), executed++);
     for (const key in mainList) {
      let truD = autoGlobe.trueVfalse(mainList[key]);
...
var seoTwitter = function (folderX, searchString, time) {
  ...
  // let seoTruth = autoGlobe.trueVfalse(idArray);
  // if (!seoTruth) {
  //   idArray = minFold
  // }
};
...
function sheetsUrls(fileX, folderX, time) {
  ...
  var runRndMat = function () {
    var rndName = autoGlobe.functionRegistry.fileList;
    let nameRnd =
...
function spreadSheetCreate(fileX, sheetName, rowHeaders, data, time) {
  ...
  try {
    ...
    if (ss && ss.getUrl()) {
      ...
      if (!data) {
        var data = JSON.parse(
          convertToObjects([[autoGlobeFN.paramsList[Math.floor(Math.random() * autoGlobeFN.paramsList.length)].name]], ["name"], start),
        )[0];
      }
      console.log(
        "boilerplate sheetParameters: line 1706\nspreadSheetCreate(data: " +
