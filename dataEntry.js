  ...
  const fieldRange = ["C8","F8","C10","F10","C12","F12","F14","C18","C17","C14"];


function createNewRecord() 
  {const settingsWS = ssGetSheetBySpreadsheetUrl("https://docs.google.com/spreadsheets/d/1-vNcN0vCLcXgMY9uwcKukUgv_4njggRZ6fqoZs-hBFE/edit#gid=138098962", "Invoice Settings");
  const formWS = ssGetSheetBySpreadsheetUrl("https://docs.google.com/spreadsheets/d/1-vNcN0vCLcXgMY9uwcKukUgv_4njggRZ6fqoZs-hBFE/edit#gid=138098962", "Invoice Form");
  const dataWS = ssGetSheetBySpreadsheetUrl("https://docs.google.com/spreadsheets/d/1-vNcN0vCLcXgMY9uwcKukUgv_4njggRZ6fqoZs-hBFE/edit#gid=138098962", "General Work Invoice");
  const delivery = Math.ceil(Math.ceil(minutePoint(formWS.getRange("C14").getValue(), formWS.getRange("C12").getValue())) / 60);
...
function saveRecord() 
  ...
  console.log(typeof formMiles + " /// " + [formMiles].length + "//////\n " + destMiles + "/////////\n " + startMiles)
  if ([formMiles].length === 1) {
    const delivery = Math.ceil(Math.ceil(minutePoint(destMiles, startMiles )) / 60);
    const deliveryCell = formWS.getRange("C17");
      deliveryCell.setValue(delivery);
  }
//   const idCell = formWS.getRange("C3");
...
authLogic(recordsFound.length !== 0)? idCell.setValue(recordsFound[0][10]):"";
...
function pdfTimesheet() 
  {saveAsPDFToFolder("https://docs.google.com/spreadsheets/d/1-vNcN0vCLcXgMY9uwcKukUgv_4njggRZ6fqoZs-hBFE/edit#gid=138098962", "Timesheet");}
...
function saveAsPDFToFolder(url, sheetname)
  ...
  for (var i = 0; i < sheets.length; i++)
      {if (sheets[i].getName().includes(pdfFilename) === false)
        {sheets[i].hideSheet()
        console.log("Is " + sheets[i].getName() + " hidden?" + " " + sheets[i].isSheetHidden())}
      else
        {sheets[i].showSheet()
        console.log("Is " + sheets[i].getName() + " hidden?" + " " + sheets[i].isSheetHidden())}
      }
