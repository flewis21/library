var coSort = function (time) {
    ...
    var secSheet = ssGetSheetBySpreadsheetUrl("https://docs.google.com/spreadsheets/d/1-vNcN0vCLcXgMY9uwcKukUgv_4njggRZ6fqoZs-hBFE/edit#gid=138098962", "sec");
    ...
    var titleArrData = ["e","t","a","o","n","r","i","s","h","d","l","f","c","m","u","g","y","p","w","b","v","k","x","j","q","z"];
    var tickerArrData = ["e","t","a","o","n","r","i","s","h","d","l","f","c","m","u","g","y","p","w","b","v","k","x","j","q","z"];
    var i = 0
...
var edgarData = function(text) 
  ...
  let h = {};
  var tmp = []
  let jsonData = Utilities.jsonParse([urlDataSource(text, null, {muteHttpExceptions:true})]);
    let objParts = {}
    for (var i = 0; i < jsonData.length; i++)
     {//Object.fromEntries(JSON.stringify(
      for (var [key, { title }] of  Object.entries(jsonData[i]))
        {objParts[title] = key}}//.map(entry => [entry[1]]))
  var arrData = covArrays(jsonData)
  if (arrData) 
    {const keys = Object.keys(arrData);
    keys.forEach(function(key) {tmp[key] = arrData[key]});}
  ...
  html.opt = tmp.map(function(r){ return "<option>" + JSON.stringify(r[1][2]) + "</option>"; }).join("");
...
function()
  ...
  linkFollow.href = url + "?func=oldSEC" + "&args=" + encodeURIComponent(uname);
  linkFollow.id = "linkFOLLOW";
  linkFollow.target = "_self";
  document.body.appendChild(linkFollow);
  document.getElementById("linkFOLLOW").click();
  document.getElementById("username").value = "";
  document.getElementById("linkFOLLOW").remove();})()}; //Global object closed
...
var opt = function(searchString) {
  ...
  const matches = []
  const alTheCo =  arrData.sort((a, b) => a - b).filter(
      (ac)=> {
          var logAC = ac[0]//["title"]
          if (JSON.stringify(logAC).toLowerCase().includes(searchString || [`group bank semi fact bio science block chain space coin`].toString().split(" ")[Math.floor(Math.random() * (Math.floor([`group bank semi fact bio science block chain space coin`].toString().split(" ").length)))]))
              matches.push(ac)
        })
  jo.user = dataArray;
  var coTable = arrData.map(
    (r)=> {
...
var randomUtility 
  = function(e, arrD, time) {// var utilStr 
  // = skyNeed(namedVar, time);var utilSubst 
  // = randomSubstance(0,4,namedVar,time)
  var rndObj 
  = needUtility([e].join(" "), arrD, time)[0];console.log("randomUtility: \nvar " + rndObj + " = needUtility("+ [e].join(" "), arrD, time + ")[0]")
  if (rndObj) {
  return {playlist: rndObj.playlistArr,cik: rndObj.rndCik,ticker: rndObj.rndTicker,title: rndObj.rndTitle,secData: rndObj.secUrl,youtubeID: rndObj.videoItem,youtubVideo: rndObj.videoItemUrl}}
  else {
    return}}

function videoSEC() {
  const ws = ssGetSheetBySpreadsheetUrl("https://docs.google.com/spreadsheets/d/1-vNcN0vCLcXgMY9uwcKukUgv_4njggRZ6fqoZs-hBFE/edit#gid=138098962", "sec");
  // const wsData = sliceValues(ws.getDataRange().getValues(),1)
  const vidOptionsCik = ws.getRange(1,1,ws.getRange("A2").getDataRegion().getLastRow(),1).getValues();
  const vidOptionsTicker = ws.getRange(2,2,ws.getRange("B2").getDataRegion().getLastRow(),1).getValues();
  const vidOptionsTitle = ws.getRange(3,3,ws.getRange("C2").getDataRegion().getLastRow(),1).getValues();
  const vidOptions = {
  ...
  // const uniqueCo = JSON.parse([]);
    ...
    var itemId = sectionCo[Math.floor(Math.random() * (Math.floor(sectionCo.length)))]
    console.log(itemId)
  ...
  function()
    ...
    var uname = document.getElementById("username").value;
    var linkFollow = document.createElement("a");
    linkFollow.href = url + "?func=dtlsInvestor" + "&args=" + encodeURIComponent(uname);
    linkFollow.id = "linkFOLLOW";
...
var yahooSort = function (ticker,time) {
  // var ticker = "AAPL"
    var data = [urlDataSource("https://query2.finance.yahoo.com/v10/finance/quoteSummary/" + ticker + "?modules=assetProfile%2CsummaryProfile%2CsummaryDetail%2CesgScores%2Cprice%2CincomeStatementHistory%2CincomeStatementHistoryQuarterly%2CbalanceSheetHistory%2CbalanceSheetHistoryQuarterly%2CcashflowStatementHistory%2CcashflowStatementHistoryQuarterly%2CdefaultKeyStatistics%2CfinancialData%2CcalendarEvents%2CsecFilings%2CrecommendationTrend%2CupgradeDowngradeHistory%2CinstitutionOwnership%2CfundOwnership%2CmajorDirectHolders%2CmajorHoldersBreakdown%2CinsiderTransactions%2CinsiderHolders%2CnetSharePurchaseActivity%2Cearnings%2CearningsHistory%2CearningsTrend%2CindustryTrend%2CindexTrend%2CsectorTrend", {muteHttpExceptions:true})]
    ...
    for (var key in uniqueKey) {
      ...
      currencySymArray.push(uniqueKey[key]["result"][0]["price"]["currencySymbol"])
