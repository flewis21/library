...
var dtlsCapital = function(url) {
  console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + "\n!" + url + ", = " + !url)
  if (url) {
    const videoSearch = [urlDataSource(url, null, {muteHttpExceptions:true, mode:"no-cors"})];
...
var forObjects = function(data, delimiter){
  ...
  json = covObjects(objectHeaders(data, JSON.stringify(i))[1], objectHeaders(data, JSON.stringify(i))[0])}
  return json
  // console.log(json[1])
...
async function jsonFormatter() {
  console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name)
  var res = UrlFetchApp.fetch("https://ordspub.epa.gov/ords/pesticides/ppls/66551-1");
  ...
  catch (err) {return res.getContentText();}}
  //console.log(obj)
...
var jsonXpath = function(jsonXpathDataRange) {
  console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + "\n!" + jsonXpathDataRange + ", = " + !jsonXpathDataRange)
  var jsonXpathNoHeaders = [jsonXpathDataRange].map(function (val) {
    return val.toString().toLowerCase();});
  var jsonXpathRowsToReturn = jsonXpathNoHeaders.filter(function (a) { return a[0]; });
...
var needCapital = function(searchString) {
  ...
  const videoSearch = [urlDataSource(`https://flewis21.github.io/foobar`, null, {muteHttpExceptions:true})];
  const table = videoSearch.slice(videoSearch.indexOf("SERP")).toString().split("SERP")
  const pong = table.map((ping)=>{return ping.substring(0)})
  return pong.toString().split(",")}
...
var rowsToReturn = function(data, index) {
  console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + "\n!" + data + ", = " + !data)
  if (data) {
    var rowsToReturn = data.filter(function (a) { return a[index]; });
    return {
      myData: rowsToReturn}}}

var tabData = function(url, xpath, headers){
  console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + "\n!" + headers + ", = " + !headers)
  var test = covObjects(jsonDataX(sliceValues(jsonXpath(url, xpath), 0)), dataHeadings(testData(headers)))
  return test}
...
var tabIndex = function(url, xpath, index, headers){
  console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + "\n!" + headers + ", = " + !headers)
  var test = covObjects(jsonDataX(sliceValues(jsonXpath(url, xpath), 0)[index]), dataHeadings(testData(headers)))
  return test}
...
var tempArrayX = function(url, indexParsed, xpath, delimiter, index) {
  console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + "\n!" + url + ", = " + !url)
  const jsonData = urlDataSource(url); try {
  const testIndex = splitX(JSON.parse(sliceValues(jsonData, indexParsed)), xpath, delimiter);}catch(error) {return}
  // const arrayObjData = testArray(testIndex) 
  const objTest = forArray(testIndex);
