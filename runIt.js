...
function getBreakthrough(e)
  ...
  if (result === 0)
  {list.push([JSON.stringify(i)*percent + " " + "out of" + " " + JSON.stringify(i) + " " + "odds!"])
  if ( JSON.stringify(i) >= 100)
  {break}}}
  list.map(function(r){ return "<li>" + r[0] + "</li>"; }).join("");
    return list},topScript: 
  function()
...
function carbon(email)
  ...
  file = DriveApp.getFilesByName(sharedFiles).next()}} catch(e) 
  {// Access denied.
    return false;}
  // Access granted. 
  ...
  console.log(editor.getEmail())}).forEach(editor => 
...
var congressLeg = function(e) 
  ...
  var congressAPIkey = {"X-API-Key": "d2E8liJwaPel5hHe3erhvLhneEj4zOWbAGKerZSv"};
  var congressEndpoint = "https://api.congress.gov/v3/bill"
  var data = urlDataSource(congressEndpoint
  ,{"method": "get","contentType": "application/json","headers": congressAPIkey,"muteHttpExceptions": true});
  spreadSheetCreate("tableS", "usCongress", ["Last Name", "First Name", "Party", "Leadership Role"], testData(covertArr(data)), 2);
  // return data
  // var chart = Charts.newDashboardPanel().setDataTable(data)
  console.log(data)})()
  // var data = urlDataSource(congressUrl)
  var civicInfoAPI = (function() 
  {var proPublicaAPI = {"X-API-Key": "xsthr17B8JL92bsGD5LChwptJHDQRM3f7mF4mVUL"};
  var proPublicaEndpoint = "https://api.propublica.org/congress/v1/";
  var civicDataURL = "https://api.propublica.org/congress/v1/116/senate/members.json";
  var data = urlDataSource(civicDataURL
  , {"method": "get","contentType": "application/json","headers": proPublicaAPI,"muteHttpExceptions": true});
  ...
  {var cm =  uSCongress[i]
  var ssData = uSCongress.map(r => [r.last_name, r.first_name, r.party, r.leadership_role]);
  dataTableBuilder.addRow([cm.last_name, cm.first_name, cm.party, cm.leadership_role]);
  var member = contentFile("congressMembers"
  , {memberData: ssData,loadTable:
  function loadTable()
  ...
  Charts.newDashboardPanel()
    ...
    .build())
    .build(),google.charts.load("current"
  , {"packages":["table"]});
  google.charts.setOnLoadCallback(drawTable);},drawTable:
  function()
  {function drawTable() 
  ...
  table.draw(data,{showRowNumber: true, width: "100%", height:"100%"});}},tableSs:
  function()
  {var myTable = spreadSheetCreate("tableS", "usCongress", ["Last Name", "First Name", "Party", "Leadership Role"], ssData, 4);
      return myTable;},})
  console.log(member)}
      return member
