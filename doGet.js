...
function doGet(e) {validGroup();validateFolders();validateFiles();
  e? e:e = objectOfS(["parameter"],[[["func", testlt()]]], Math.floor((maxTime - new Date() % (1000 * 60)) / 1000))
  ...
  if (mis(fx)) {
    ...
    try {
      ...
      return renderTemplate(surveyPlayer(fx,error),{},content)}; if (payload){
    if (payload.length === 99 || payload.length ===94 || payload.length === 83 || payload.length ===97 || payload.length ===101 || payload.length ===103 || payload.length ===136 || payload.length ===132 || payload.indexOf("&entry") > -1) {
      return renderTemplate(mis(payload),{},"All departments")}
    else {
      return renderTemplate(`<!DOCTYPE html><html><head><base target="_self"></head><body class="amber"><div class="amber" id="div">${payload}</div></body></html>`,{},"All departments")}}}
    ...
    var aVar = objectOfS(["parameter"],[[["func", arguments.callee.name]]], Math.floor((maxTime - new Date() % (1000 * 60)) / 1000))
    ...
    console.log(Math.floor((maxTime - new Date() % (1000 * 60)) / 1000) + "\n" + arguments.callee.name + "\nfx is !" + !fx + ", = " + fx)
      ...
      const sheet = ssGetSheetBySpreadsheetUrl("https://docs.google.com/spreadsheets/d/1-vNcN0vCLcXgMY9uwcKukUgv_4njggRZ6fqoZs-hBFE/edit#gid=138098962", "Receive");
...
function handleGetData() {
  var data = { message: "Hello from Apps Script Backend!", timestamp: new Date() };
...
function handleFormSubmission(e) {
  try {
    ...
    return ContentService.createTextOutput(JSON.stringify({ success: false, message: "Error submitting form." })).setMimeType(ContentService.MimeType.JSON);
  }
}
...
function misBing(e, time) {
  ...
  console.log(JSON.stringify(e));
    ...
    const randomKey = Math.floor(Math.random() * (Math.floor(uniqueCoArray.length)))// Math.floor(Math.random())
...
function()
  ...
  const confirmation = window.confirm("Click OK to continue to the destination."); 
  if (confirmation) {
  ...
  function() {
    const serverSide = function (func, args) {
      ...
      .withFailureHandler(error => {
        console.log(document.getElementById("test").innerHTML)
        reject(error)})
      .runBoilerplate(func, args)})}; //serverSide closed
// VideoPlayer Widget
serverSide("userClicked")
  .then((videoSearch) => {
    document.getElementById("vids").innerHTML = videoSearch}) //Global then closed
  .catch((error) => {
    alert(error)
    console.log(error)})}, //vidApp closed //Global catch closed    
...
html.jsApp = 
  ...
  .then((res) => {
    ...
    for (var i = 0; i <input.length; i++) {
      ...
      var companyHtml = "<div><p><em> Hello Scroll!<em>,<br><span> <div id='searchMe'>${currentTitle}</div> <h2>${currentTicker}</h2> is ready to trade. </span><br>Best Regards.<br>P.S. <a href='https://www.sec.gov/edgar/browse/?CIK=${currentCik}&owner=exclude' target='_blank' rel='noopener noreferrer'>Click here for info!</a></p></div>"};
    document.getElementById("template1").innerHTML = companyHtml;}) //Global then closed
  .catch((error) => {
    alert(error);
    console.log(error)})}, //jsApp closed; //Global catch closed
html.myRandoms = randomTitle
return html.evaluate().getContent()} //webApp closed    // })Global object closed
...
var runAll =  function(func, args) {try {
  var arr 
    = func.split(".");
  var libFunc 
    = arr[0];}catch(error) {return};
  args 
    = args || [];
  return this[libFunc].apply(this, args)}
