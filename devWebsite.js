...
var devWebsite = function(e) {
  ...
  case "epa": // <!----------------------------------JSON API Results---------------------------------!>
      ...
      webApp = epaA(edgarData("https://www.sec.gov/files/company_tickers.json", "Edgar", dateTime().getContent()).getContent(), epaA("https://ordspub.epa.gov/ords/pesticides/ppls/", epaB("https://ordspub.epa.gov/ords/pesticides/ProductSearch/searchWithIngName/v1/", epaC("https://ordspub.epa.gov/ords/pesticides/pplstxt/", epaD("https://ordspub.epa.gov/ords/pesticides/cswu/ProductSearch/partialprodsearch/v2/riname/", "c", "items/", 6, "/productname", "/"), "items/0/active_ingredients/0/active_ing", "/"), "items/0/eparegnumber", "/")));
      webApp.url = getUrl(ScriptApp);
      return contentApp(webApp)//:contentFile('uiAccess');
  default: // <!-----------------------------Default Website-------------------------------!>
  const unData = JSON.stringify([urlDataSource("https://www.sec.gov/files/company_tickers.json")])
  const testUnData = untestedData(unData);
  const jsonUnData = JSON.stringify(testUnData)
  const rowsDefault = jsonXpath([jsonUnData]);
  let search = rowsDefault.filter(function(a) { return a[0] == query; } )
  let searchEntries = [search].values().next().value
  for (let key in search) {console.log([search].filter(function (a) { return a[1] == query; }))}
  ...
  webApp.thrift = jsonINIT('https://drive.google.com/drive/u/0/shared-with-me');
  ...
  const jsData = covObjects(testData([jsonUnData]), [["id"],["ticker"],["title"]]);
  console.log(jsData)
  // const widgetData = JSON.stringify(covObjects(testData(jsData), [["id"],["ticker"],["title"]]));
  const baseUrl = getUrl(ScriptApp);
  const inventoryUrl = getUrl(ScriptApp) + "?webApp=request&sheetName=Inventory&headers=0&q="//.concat(rows,);
  console.log(rowsDefault[0])
  const financeUrl = getUrl(ScriptApp) + "?webApp=jsonXpath&q=" + encodeURIComponent(rowsDefault[0][1]);
  navBar = navBar(baseUrl, "Social", financeUrl, "Finance", instaUrl, "Instagram", tweetUrl, "Twitter", baseUrl, "Social", groupsUrl, "Google Groups", bloggerUrl, "Blogger", inventoryUrl, "Inventory", baseUrl, "Social").getContent();
  dateTime = dateTime()
