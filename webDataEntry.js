...
var storeFunction = function (xfactor) {
  ...
  if (xfactor) {
    if (arrayJobs.indexOf(xfactor) > -1) {
      if ([xfactor].join("").toLowerCase() === ["SKU"].join("").toLowerCase()) {
        var booleFactor = [];
        JSON.parse(arrayJobs).map((dx) => {
          var neatR = dx["SKU"];
          console.log(typeof neatR === "string");
      ...
      if (
        [xfactor].join("").toLowerCase() ===
        ["Pack Size"].join("").toLowerCase()
      ) {
        var booleFactor = [];
        JSON.parse(arrayJobs).map((dx) => {
          var neatR = dx["Pack Size"];
          console.log(typeof neatR === "string");
          if ([neatR].join("").length > 0) {
            booleFactor.push(neatR);
          }
      ...
      if ([xfactor].join("").toLowerCase() === ["QTY"].join("").toLowerCase()) {
        var booleFactor = [];
        JSON.parse(arrayJobs).map((dx) => {
          var neatR = dx["QTY"];
          console.log(typeof neatR === "string");
      ...
      if (
        [xfactor].join("").toLowerCase() ===
        ["COST PER PACK"].join("").toLowerCase()
      ) {
        var booleFactor = [];
        JSON.parse(arrayJobs).map((dx) => {
      ...
      if (
        [xfactor].join("").toLowerCase() ===
        ["COST PER PIECE"].join("").toLowerCase()
      ) {
        var booleFactor = [];
        JSON.parse(arrayJobs).map((dx) => {
      ...
      if (
        [xfactor].join("").toLowerCase() ===
        ["Last Modified"].join("").toLowerCase()
      ) {
        var booleFactor = [];
        JSON.parse(arrayJobs).map((dx) => {
