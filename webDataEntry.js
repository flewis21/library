...
class XStation {
  constructor() {
    this.ws = ssGetSheetBySpreadsheetUrl(
      "https://docs.google.com/spreadsheets/d/1-vNcN0vCLcXgMY9uwcKukUgv_4njggRZ6fqoZs-hBFE/edit#gid=138098962",
      "General Work Invoice",
    );
    this.car = this.ws
      .getRange(2, 2, this.ws.getRange("B2").getDataRegion().getLastRow(), 1)
      .getValues();
    this.jobType = this.ws
      .getRange(3, 2, this.ws.getRange("C2").getDataRegion().getLastRow(), 1)
      .getValues();
    this.vin = this.ws
      .getRange(4, 2, this.ws.getRange("D2").getDataRegion().getLastRow(), 1)
      .getValues();
    this.data = sliceValues(this.ws.getDataRange().getValues(), 1);
    this.list = [];
    for (var i = 0; i < this.data.length; i++) {
      this.list.push([this.data[i][1] + " " + this.data[i][2] + " - Vin/:" + this.data[i][3]]);
    }
  }
}
let cXStation = new XStation();
    ...
    ContentApp.appContent(
      `<html id='test'>
        <body>
          ...
          <?!= stylesSleep.wDERunIt.getContent() ?>
          <input type='hidden' value='<?= breakUrl ?>' id='url' />
        </body>
      </html>`,
      {
        breakUrl: getUrl(ScriptApp),
        list: cXStation.list.map(function (r) {
          return r[0];
        }),
        // runIt: function () {
